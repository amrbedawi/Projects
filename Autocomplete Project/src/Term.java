...
public class Term implements Comparable<Term> {
	...
	public Term(String word, double weight) {
		...
		if(word == null) throw new NullPointerException("Parameter Type cannot be null");	
	...
	public static class PrefixOrder implements Comparator<Term> {
		...
		public int compare(Term v, Term w) {
			...
			if((v.myWord.length() < this.myPrefixSize) || (w.myWord.length() < this.myPrefixSize)) {
				for(int k = 0; k < Math.min(v.myWord.length(), w.myWord.length()); k ++) {
					if(v.myWord.charAt(k) - w.myWord.charAt(k) != 0) return v.myWord.charAt(k) - w.myWord.charAt(k);
				}
